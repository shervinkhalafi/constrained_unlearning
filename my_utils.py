...
class my_DDIMScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        if eta > 0:
            ...
            #new
            mu = prev_sample.clone().detach()
            #new
            prev_sample = prev_sample + variance

            # Fix: Ensure log_prob is properly calculated and has correct shape
            if return_log_prob:
                log_prob = -0.5 * (
                    torch.sum((prev_sample - mu)**2, dim=[1,2,3])
                )
                # Ensure log_prob has the right shape for accumulation
                if log_prob.dim() == 0:
                    log_prob = log_prob.unsqueeze(0)


        if not return_dict:
            if return_log_prob == True:
                #new
...
def get_text_embedding(prompt, tokenizer, text_encoder, accelerator):
    text_input = tokenizer(
        prompt, padding="max_length", max_length=tokenizer.model_max_length, truncation=True, return_tensors="pt"
    )
    # Move to device before passing to text_encoder to avoid multi-GPU issues
    input_ids = text_input.input_ids.to(accelerator.device)
...
def sample_loop(accelerator, scheduler, unet, vae, tokenizer, text_encoder, prompts, num_inference_steps=10, guidance_scale=7.5, generator_seed=None, with_KL = False, with_log_prob = False, grad = False, precomputed_text_embeddings=None, precomputed_uncond_embeddings=None):
    ...
    if generator_seed is not None:
        # Use CPU generator to avoid multi-GPU synchronization issues
    ...
    with context_manager:
        ...
        log_probs_accumulator = torch.zeros(1, requires_grad = True).to(accelerator.device)
